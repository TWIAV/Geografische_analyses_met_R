...
date: "3 juni 2017"
header-includes:
   - \providecommand{\versionnumber}{0.4.9}
...
\begin{tcolorbox}[enlarge by=5mm, hyphenationfix, title=Dit is eigenlijk een \textbf{e-boek}...]
Dit PDF document is zo opgemaakt dat het dubbelzijdig kan worden afgedrukt op A4 formaat.\par
Maar bedenk - voordat je gaat printen - eerst welke nadelen een gedrukte versie allemaal heeft:
\begin{itemize}
\item \textbf{Geen Ctrl+F:} op papier kun je veel minder makkelijk zoeken dan in een electronisch document. (Er is wel een inhoudsopgave, maar er staat geen index achter in het boek...)
\item \textbf{Geen Ctrl+C/Ctrl+V:} vanaf papier kun je geen codevoorbeelden knippen en plakken; je zal alle code zelf in moeten typen. (Maar of dat nou echt een nadeel is...?)
\item \textbf{Hyperlinks werken niet op papier:} dit document staat vol verwijzingen - zowel interne als externe - die in de electronische versie wel met ??n klik te volgen zijn.
\item \textbf{Letters hebben een vaste grootte:} op papier kun je de lettergrootte niet aanpassen - en dat kan vooral voor de oudere lezertjes onder ons best een probleem zijn.
\end{itemize}
Kortom: het welgemeende advies is om dit boek te lezen met een PDF reader op je favoriete \textit{device}. En op het moment dat je echt aan de slag gaat: gewoon op de computer waarop je ook \textbf{R} en \textbf{RStudio} hebt draaien.\par
(En als je dan toch gaat afdrukken, overweeg dan om dat wel in zwart-wit te doen.)
\end{tcolorbox}

Dit is een uitgave van [TWIAV - Onderzoek & Advies](http://www.twiav.nl/).
...
\begin{tcolorbox}[enlarge by=5mm, hyphenationfix, title=Oproep: beoordelaars gezocht]
Geen enkel goed boek kan zonder meelezers. Dus als je je geroepen voelt om (een deel van) het boek grondig te beoordelen en van commentaar voorzien - zowel tekstueel als codetechnisch - dan houdt de auteur zich van harte aanbevolen.\par
...
Om deze cursus te volgen heb je een machine nodig. Ja, h?, h?. Maar wacht, dat hoeft niet per se je eigen computer te zijn, dat kan ook een virtuele machine zijn.

#### Virtuele machine van OSGeo-Live


Wij raden je aan om eens te kijken naar de *Virtual Machine* van [OSGeo-Live](http://live.osgeo.org/en/index.html). Dit is een machine waarop een grote verscheidenheid aan *open source* (en dus gratis) geografische applicaties ge?nstalleerd staat. Het is, zeg maar, de reclame-DVD van [The Open Source Geospatial Foundation](http://www.osgeo.org/).
...
(**R** staat al ge?nstalleerd op **OSGeo-Live**; **RStudio** moet je zelf even toevoegen.)
...
Het wordt ten sterkste aangeraden om de **R** scripts in deze cursus regel voor regel uit te voeren (en niet meteen in ??n keer in zijn geheel). Probeer vooraf te bedenken wat er volgens jou gaat gebeuren, en controleer na elke regel of dat ook inderdaad zo is. 
...
Tip: zoek - voordat je een vraag stelt in een discussiegroep - uit of deze niet al eens gesteld en beantwoord is. Die kans is namelijk groot. Als je met heel specifieke zoektermen googlet dan vind je het antwoord misschien wel in de archieven van ??n van de de onderstaande mailinglijsten: 
...
Al kort na het uitbrengen van een vroege versie in 1994 sporen collega's van over zee hen aan om de broncode van deze nieuwe programmeertaal vrij te geven. Aangezien ze inschatten dat de vooruitzichten van **R** als commerci?el product slechts beperkt zijn, brengen ze hun geesteskind inderdaad uit als 'vrije software', onder de GPL licentie van de *Free Software Foundation*. Vanaf dat moment kan **R** gratis gedownload worden. Ook wordt er een discussielijst opgezet, waarop gebruikers over de verdere ontwikkeling van **R** mee kunnen praten.
...
En ook de release die jij hebt ge?nstalleerd  heeft een bijzonder codenaam. Deze verschijnt in de console als je \textbf{R} opstart.
\end{tcolorbox}

Tijdens het installatieproces kun je - afhankelijk van het besturingssysteem - kiezen of je de 32-bit of de 64-bit versie wilt installeren. Als je geen keuze maakt, dan worden beide versies ge?nstalleerd.
...
Let op: **RStudio** is geen vervanging, maar draait bovenop **R**. Dus je moet er voor zorgen dat **R** eerst is ge?nstalleerd voordat je de studio toevoegt.
...
**R** is modulair opgebouwd. Er is het basissysteem, zoals we dat in de vorige paragrafen hebben ge?nstalleerd. Maar daar bovenop zijn er vele aanvullende bibliotheken, zogeheten *packages*, beschikbaar.
...
Tijdens een presentatie in oktober 2009 kon Ross Ihaka nog 'opscheppen' dat er al meer dan 1.700 *packages* aan het **CRAN** waren toegevoegd ([Ihaka, 2009](https://www.stat.auckland.ac.nz/~ihaka/downloads/Massey.pdf)), maar nu - op 22 november 2015, dus zes jaar later - staat de teller al op het aantal van 7.512 bijdragen (Zie: <https://cran.r-project.org/web/packages/>), en de teller loopt door.     
...
In de volgende hoofdstukken zullen we telkens ??n of meer van deze bibliotheken introduceren. De rode draad in de rest van het boek is dan ook: *Packages, packages, packages...* (Overigens heeft deze handleiding niet de pretentie om alle 'geografische' bibliotheken te behandelen. Er is altijd meer dan men in ??n boek kan samenvatten.)
...
Om een bibliotheek te kunnen gebruiken, moet je hem eerst - ??nmalig - installeren. Dit kan heel eenvoudig vanuit **RStudio** zelf. Bijvoorbeeld:

```{r eval=FALSE}
install.packages("rgdal") # bibliotheeknaame met aanhalingstekens
```

Zoals je ziet wordt de benodigde software automatisch gedownload en ge?nstalleerd.
...
In **R** scripts zie je vaak bovenaan ??n of meerdere `library()` statements staan, zodat de benodigde bibliotheken geladen worden voordat de rest van het script wordt uitgevoerd.

### Zelf een bibliotheek bouwen: `devtools` \label{dvtls}

Het is mogelijk om zelf een **R** *package* te bouwen en dit via het **CRAN** met de weredwijde gemeenschap te delen. Misschien dat je nu denkt: 'Wat? Ik ben nog nauwelijks met **R** begonnen, en nu wil je al direct dat ik een bibliotheek ga bouwen?!' Nee, nee, nee: ik wil niks. Maar wellicht krijg je de smaak te pakken, en heb je op een dag wel de behoefte om een eigen *package* te cre?ren.

Overigens hebben we gezien dat er al heel veel *packages* zijn. Dus als je iets wil dat met **base R** niet lijkt te kunnen, kijk dan - voordaat je zelf iets bouwt - zeker even rond of er niet al een bibliotheek is die de gevraagde functionaliteit wel biedt. 

Het *package* [`devtools`](https://cran.r-project.org/web/packages/devtools/index.html) biedt gereedschap om het bouwen van een **R** bibliotheek gemakkelijker te maken.
...
Doorloop deze procedure ??n of meerdere keren (en lees de commentaarregels, die beinnen met een *#*), en kijk na elke stap wat er gebeurd is. Gebruik `?` voor uitleg over een functie. Bijvoorbeeld: `?unzip`, `?unlink` of `?rm`.
...
\begin{tcolorbox}[enlarge by=5mm, hyphenationfix, title=De bocht in de Maas]
    ...
    \vspace*{0pt}
        ...
        Tegenwoordig is Pebesma hoogleraar aan de \textbf{Westf?lische Wilhelms-Universit?t (WWU)} in M?nster (Duitsland), waar hij directeur is van het \textbf{Institut f?r Geoinformatik}.
    \end{minipage}\hfill%
...
In deze paragraaf gaan we kennismaken met ??n van de hiervoor genoemde ruimtelijke datastructuren. We gaan zelf een SpatialPointsDataFrame aanmaken, op basis van gegevens in een data.frame.

Begin met het installeren van de betreffende bibliotheek en laad deze in het geheugen:

```{r eval=FALSE}
install.packages("sp") #??nmalig
library(sp)
```
...
Dit kunnen we oplossen door niet ??n generieke positie (`pos = 4`) voor alle labels op te geven, maar door een vector aan te maken waarin ieder museum een eigen labelpositie krijgt, zodat we kunnen vari?ren.
...
Het SpatialPointsDataFrame dat we in de vorige paragraaf (\ref{spdf}) hebben aangemaakt, is nog niet helemaal compleet, omdat we er nog geen co?rdinaatreferentiesysteem (CRS) aan hebben toegekend. Het Frame heeft weliswaar co?rdinaten, maar we hebben nog niet vastgelegd hoe deze aan een plek op aarde te relateren zijn.

Er zijn twee soorten co?rdinaatsystemen:
...
Een bekend, en tegenwoordig alom gebruikt systeem van het eerste soort is WGS84. Het offici?le Nederlandse systeem - het Rijksdriehoeksstelsel (zie paragraaf \ref{RDM}) - is een voorbeeld van het tweede type.
...
Het was de \textbf{European Petroleum Survey Group} die het initiatief heeft genomen de vele wereldwijd gebruikte co?rdinaatsystemen elk van een eigen, unieke code te voorzien, om de communicatie over locaties te vergemakkelijken. Het moge duidelijk zijn dat de (financi?le) belangen van de olieindustrie hier groot zijn; men kan het zich simpelweg niet permitteren om op een verkeerde plek te gaan boren.\par
...
Nu is natuurlijk de vraag: welk co?rdinaatsysteem gaan we aan onze data koppelen? Op die vraag is maar ??n antwoord mogelijk, namelijk: het juiste CRS. Je kunt immers niet zomaar een willekeurige referentie aan de gegevens hangen.
...
De `x` en `y` van onze Amsterdamse museumdataset zijn gegeven in Nederlandse RD-co?rdinaten. De *EPSG* code van dit systeem is 28992.
...
### Het Nederlandse co?rdinaatsysteem: het stelsel van de Rijksdriehoeksmeting \label{RDM}

In Nederland worden de meeste geografische data uitgewisseld in RD-co?rdinaten. De EPSG code voor het RD-stelsel is 28992.

Het Rijksdriehoeksstelsel is het nationale geodetische triangulatiesysteem van Nederland. De basis van dit systeem is een stereografische kaartprojectie gecentreerd op de spits van de Onze Lieve Vrouwetoren in Amersfoort. Op het zo verkregen projectievlak is een cartesiaans assenstelsel geconstrueerd. De voor het RD-stelsel gebruikte stereografische projectie is gebaseerd op de ellipso?de van Bessel (1841). Een belangrijk voordeel van de projectie is, dat deze *conform* (hoekgetrouw) is: een hoek gemeten in de kaart is gelijk aan de werkelijke hoek. Voor een klein gebied als Nederland kan men gebruik maken van een dergelijke projectie, al nemen de afstandsfouten wel toe, naarmate men verder van de oorsprong verwijderd raakt.

De oorsprong van het systeem ligt in Amersfoort. Aanvankelijk was deze oorsprong ook het nulpunt (X=Y=0) van het systeem. Dit betekende dat men in het noordoosten van Nederland met positieve X- en Y-co?rdinaten kon werken, maar dat men in de andere delen van het land rekening moest houden met een negatieve X- en/of een negatieve Y-co?rdinaat. Om dit probleem op te lossen heeft men in het begin van de jaren zeventig van de twintigste eeuw het nulpunt verschoven, en wel 155 kilometer naar het westen en 463 kilometer naar het zuiden. Nederland ligt nu in zijn geheel in het eerste kwadrant van het assenstelsel, en door de extreem grote Y-verschuiving is de grootst voorkomende X-waarde nog altijd kleiner dan de kleinst mogelijke Y-waarde. Daardoor zijn alle voorkomende co?rdinaten nu positief, en de X- en Y-waarden zijn goed uit elkaar te houden (zie figuur \ref{afbRD}).

Het geldigheidsgebied voor de RD-co?rdinaten is nauwkeurig gedefinieerd. Globaal mogen deze co?rdinaten alleen gebruikt worden op het Nederlandse vasteland en in een kleine band daaromheen. Daar valt niet het gehele Nederlandse Continentaal Plat (het Nederlandse deel van de Noordzee) binnen. In Belgi? en Duitsland maakt men gebruik van andere co?rdinaatsystemen.
...
\item alle co?rdinaatgetallen zijn positief
\item Y-co?rdinaat is altijd groter dan X-co?rdinaat
...
\caption{De co?rdinaten van het Rijksdriehoeksstelsel}
\label{afbRD}
\end{figure}
...
In de voorgaande paragrafen hebben op de volgende manier een SpatialPointsDataFrame gecre?erd:
...
2. We gaan een alternatieve syntaxis gebruiken voor het aanwijzen van de co?rdinaten.
...
Doordat het object `mus` op een andere manier is aangemaakt dan `ams.spdf`, is er 'diep onder water' een klein technisch veschil tussen de beide dataframes\footnote{Nieuwsgierig naar dit verschil? Ga op onderzoek uit met {\small\ttfamily{all.equal(ams.spdf, mus)}}}. Maar voor praktische toepassingen zijn de objecten gelijkwaardig: het betreft in beide gevallen een geldig **SpatialPointsDataFrame**. Dit kun je controleren door de samenvatting op te vragen: `summary(ams.spdf)` en `summary(mus)`.
...
Bijvoorbeeld: wij willen later in deze handleiding onze Amsterdamse musea op een OpenStreetMap kaart tonen (zie hoofdstuk \ref{chLflt}). Hiervoor is het noodzakelijk dat de co?rdinaten worden getransformeerd naar WGS84. Gelukkig kennen wij de *EPSG* code van dit CRS, namelijk EPSG:4326.

In het voorbeeld hieronder maken we een nieuw object aan, waarbij de co?rdinaten uit `ams.spdf` worden overgezet naar WGS84. Bekijk de `coordinates` van `ams.spdf.wgs84`: dit zijn nu geen meters meer, maar decimale graden.
...
# Het inlezen van geografische gegevens in R \label{inlzn}
...
**GDAL** staat voor *Geospatial Data Abstraction Library*. Deze bibliotheek biedt abstracte gegevensmodellen voor geografische gegevens, ??n voor het raster- en ??n voor het vectorformaat. Daarnaast bevat **GDAL** een hoeveelheid programma's, zoals bijvoorbeeld `ogr2ogr`, `ogrinfo`, `gdaldem` en `gdal_contour`, voor het bewerken van geografische data. Deze programma's zijn zogeheten *command line utilities* (dat wil zeggen: ze moeten via de commandoregel worden aangesproken) en bieden vele opties voor onder andere:
...
Je ziet nu in de console dat de gevraagde bibliotheek automatisch wordt gedownload en ge?nstalleerd. Overigens maakt `rgdal` gebruik van ruimtelijke klassen die in de *package* `sp` zijn gedefini?erd. Deze afhankelijkheid wordt automatisch gedetecteerd. Dus mocht je `sp` nog niet hebben ge?nstalleerd, dan verschijnt in de console de melding: `also installing the dependency 'sp'`.
...
summary(gem2014)
```
We zien hier dat er al een CRS aan de dataset is toegekend. Ja, nogal wiedes, deze co?rdinaatreferentiegegevens zijn overgenomen van het invoerbestand.

Als je de `proj4string` opvraagt, dan zie je dat de dataset geprojecteerd is in RD-co?rdinaten (en waarom de string in dit boek van de pagina loopt, dat weet ik nog niet...):
...
De twee tabellen zitten samen in ??n zipbestand, dat we willen downloaden en uitpakken in de submap 'Data' van onze werkmap.
...
+ *H?, het valt op dat we telkens het commando `dir.create("./Data", showWarnings = FALSE)` meegeven. Waarom is dat?*
...
# Voeg een titel en een legenda toe
title("Het spoorwegnetwerk voor reizigersvervoer, Nederland, 2015")
legend("bottomleft", legend=c("Spoorlijnen", "Stations"), cex = .7,
title="Legenda", bty="o", inset=0.05,
lty=c( 1,-1,-1), pch=c(-1,19),
col=c("black", "purple"))
```
...
## En nog een keer gegevens inlezen: nu een ESRI Shapefile \label{shp}
...
Hiervoor biedt **RStudio** een goede mogelijkheid met de [Data Viewer](https://support.rstudio.com/hc/en-us/articles/205175388-Using-the-Data-Viewer). Dit is een venster waarin je gegevens kan filteren, sorteren en doorzoeken. 

Om de attribuutgegevens van onze eerder ingelezen dataset `gem2014` te bekijken, moet je het volgende commando geven:
...
Het venster van de **Data Viewer** verschijnt nu in de linkerbovenhoek van de grafische gebruikersinterface van RStudio (zie Figuur \ref{RStudioGUI}).
...
+ En hoeveel inwoners had de gemeente S?dwest-Frysl?n op die dag?
+ Hoeveel gemeenten liggen er in de Veiligheidsregio Haaglanden?

## Selecteren

In een eerder hoofdstuk (zie hoofdstuk \ref{inlzn}) hebben we een dataset met gemeenten ingelezen - `gem2014`. In deze paragraaf gaan we bekijken hoe we een gedeelte van deze data kunnen selecteren.

### Selecteren met de functie `subset()`

#### Rijen selecteren

Stel dat je alleen de gemeenten in de provincie Limburg wilt selecteren. Dat kan met de functie `subset()` als volgt (let op het dubbele isgelijkteken):
...
View(limburg)
```
Nu gaan we een selectie uitvoeren op een numerieke kolom. We willen namelijk een overzicht van alle grote gemeenten (hier gedefini?erd als gemeenten met meer dan 200.000 inwoners):
...
De dataset met gemeenten heeft een groot aantal kolommen met attribuutgegevens, namelijk wel 34. Met de functie `subset()` kun je een de selectie van deze kolommen inperken.
...
Als je op deze manier een aantal kolommen selecteert uit een  `Spatial*DataFrame` object, dan wordt de geometrie meegeselecteerd. Dus het resultaat is ook een `Spatial*DataFrame` object.
...
Je kunt natuurljk ook een selectie maken van niet-opeenvolgende kolommen (bijvoorbeeld naam, provincie en aantal 
inwoners). En ook dat kan op twee manieren:
...
#### Rijen ?n Kolommen selecteren

Natuurlijk kun je ook een selctie maken van rijen ?n kolommen. In het voorbeeld hieronder selecteren we een aantal kolommen met betrekking tot de grote gemeenten:
...
### Selecteren met indices (*bracket notation*) \label{indsel}
...
Stel dat je alleen de gemeenten in de provincie Groningen wilt selecteren. Dat kan met vierkenate haken
...
```{r eval=FALSE}
plot(groningen, col = "darkgreen", border = "lightgray")
View(groningen)
```
...
Je kunt ook een aantal niet-opeenvolgende kolommen selecteren, naar keuze met kolomnummers of (in het voorbeeld hieronder) met kolomnamen:
...
Natuurlijk kun je op deze wijze ook een selctie maken van rijen ?n kolommen. In het voorbeeld hieronder selecteren we een aantal kolommen met betrekking tot de wel h????l erg kleine gemeenten:
...
Als je graag mooie thematische kaartjes wilt gaan maken met **R**, dan moet je beslist eens kijken naar de nieuwe *package* `tmap`. Deze bibliotheek is ontwikkeld door [Martijn Tennekes](http://www.von-tijn.nl/tijn), werkzaam bij het Nederlandse Centraal Bureau voor de Statistiek.

De ontwikkeling van deze package is begonnen in 2013 - de *Initial commit* in de [GitHub repository](https://github.com/mtennekes/tmap) dateert van 25 november van dat jaar - en op 28 mei 2015 is versie 1.0 van `tmap` vrijgegeven.

In de zomer van 2015 heeft Tennekes een [presentatie](http://user2015.math.aau.dk/presentations/20.pdf) gegeven, over `tmap`, op de [useR! Conference 2015](http://user2015.math.aau.dk/) in Aalborg, Denemarken.

In de volgende paragrafen gaan we theamtische kaarten maken op basis van de gemeenten (`gem2014`) en (`spoorwegen2015`) die we in een eerder hoofdstuk hebben ingelezen (zie hoofdstuk \ref{inlzn}).
...
\tcbset{coltitle=black, colbacktitle=white, colframe=gray!75!black,colback=white,nobeforeafter}
\begin{tcolorbox}[enlarge by=5mm, hyphenationfix, title=Een 'concurrerende' \textit{package}? {\small\ttfamily{cartography}}]
Eind oktober 2015 verscheen er een \href{https://stat.ethz.ch/pipermail/r-sig-geo/2015-October/023604.html}{berichtje} in de mailinglijst **R-sig-Geo**, waarin aandacht werd gevraagd voor een nieuwe bibliotheek: {\small\ttfamily{cartography}}.\par
Deze is ontwikkeld door Timoth?e Giraud en Nicolas Lambert van de \href{http://elementr.hypotheses.org/}{Groupe ElementR}. Let op de Franse uitspraak: \textbf{ElementR} - \textit{R pour les sciences humaines et sociales}.\par
...
\begin{itemize}
\item \textbf{Vingette} met voorbeelden: \url{https://cran.r-project.org/web/packages/cartography/vignettes/cartography.html}
\item \textbf{Handleiding op het CRAN:}  \url{https://cran.r-project.org/web/packages/cartography/cartography.pdf}
\item \textbf{Broncode op GitHub:} \url{https://github.com/Groupe-ElementR/cartography/}
\end{itemize}
\end{tcolorbox}
...
## De bibliotheek `mapplots`


### Cirkeldiagrammen op de kaart
...
*Et voil?!* De afbeelding in deze bundel is zo statisch als wat. Maar de kaart in de **Viewer** tab is echt interactief: je kunt inzoomen, uitzoomen en pannen. Vergeet ook niet op de markers te klikken om de tooltip te controleren. 
...
Deze 3 stappen kunnen worden teruggebracht tot ??n met behulp van de *pipe operator*, het symbool: `%>%`
...
Daarmee leidt het gebruik van de deze operator tot code die ?n beter leesbaar is, en makkelijker te onderhouden.

De pipe operator is een relatief nieuw fenomeen in **R**; hij bestaat pas sinds 2014 en is ontwikkeld door Stefan Milton in zijn `magrittr` package. (Nee, inderdaad: je hebt dit `magrittr` package niet zelf ge?nstalleerd - maar het is ge?mporteerd door `leaflet`, een bibliotheek die je wel hebt draaien.)
...
\caption{het 'offici?le' logo van het magrittr package}
\label{MagrittrLogo}
\end{figure}
...
## Overlapanalyse (punt in polygoon): `over()`

Met de functie `over()` uit de `sp` bibliotheek kun je overlapanalyses doen. In deze paragraaf doen we wat eenvoudige oefeningen met punten in polygonen, namelijk stations in gemeenten.

De functie `over()` kijkt naar de locatie van *object x* (het station) en haalt de attribuutgegevens op van *object y* (de gemeente).
...
Het data.frame `Gem_met_stations` bevat 398 observaties met 34 variabelen. Er zijn immers 398 stations, en voor elk station zijn alle 34 attribuutgegevens van de betreffende gemeente opgehaald. Sommige gemeenten komen meerdere keren voor, omdat er meer dan ??n station is.

OK - dit was om te laten zien hoe de functie werkt. Hieronder volgen twee praktische toepassingen.

### Verrijken van een tabel met gegevens over de locatie - met `spCbind()`

We willen graag `stations2015` uitbreiden met een attribuutkolom **Gemeentenaam**.

**Stap 1:** genereer de attribuutkolom:
...
Nu willen we deze ene kolom toevoegen aan `stations2015`. Dit kan met `spCbind()` uit de bibliotheek [`maptools`](https://cran.r-project.org/web/packages/maptools/maptools.pdf). (`spCbind` verschaft *cbind-achtige* methoden voor Spatial*DataFrame objecten.)
...
En nu willen we ook graag een attribuutkolom **Provincie_naam** toevoegen.
...
Met `over()` halen we de provincienaam bij elk station op, en met `table()` zetten we in ??n moeite door een telling van de voorkomens in een tabelletje:
...
In **R** kun met behulp van vierkante haken selecties maken van een dataset (zie voor een uitleg van de syntaxis paragraaf \ref{indsel}).

Deze syntaxis kan ook gebruikt worden voor geografische selecties.

In het voorbeeld hieonder maken we eerst een subset `brabant` aan, en vervolgens selecteren we alle stations in die provincie:

```{r}
brabant <- gem2014[gem2014$Provincie_naam == "Noord-Brabant", ]
stations_brabant <- stations2015[brabant, ]
```
De dataset `stations_brabant` bevat nu alle 35 stations stations die gelegen zijn in de Nederlandse provincie Noord-Brabant. En de dataset bevat ook de puntgeometrie van deze stations.
...
## De bibliotheek `rgeos`

In deze paragraaf gaan we gebruik maken van functies uit het *package* `rgeos`. Deze bibliotheek biedt toegang vanuit **R** tot de functionaliteit van de krachtige 'geometrie motor' **GEOS**. Voordat we de *package* gaan installeren, geven we eerst wat achtergrondinformatie over **GEOS**.

### GEOS: *Geometry Engine - Open Source*

**GEOS** staat voor *Geometry Engine - Open Source*. Het is een **API** (*application programming interface*) met functies voor het vergelijken en bewerken van geometrische objecten, zoals deze gedefini?erd zijn in de [OpenGIS Implementation Specification for Geographic information - Simple feature access - Part 2: SQL option](http://www.opengeospatial.org/standards/sfs) (de opvolger van een de eerdere **OpenGIS Simple Features Implementation Specification for SQL**) van het [Open Geospatial Consortium (OGC)](http://www.opengeospatial.org/). 

Meer informatie over **GEOS** is te vinden op de website <http://trac.osgeo.org/geos/>.

### *Spatial predicates* (Ruimtelijke kenmerken) 

Met behulp van zogeheten *Spatial predicates* gaan we de ruimtelijke relatie tussen objecten onderzoeken. Voorbeelden hiervan zijn `gTouches()`,  `gOverlaps()` en `gIntersects()`. Deze functies geven TRUE als twee objecten elkaar overlappen/raken/snijden, en FALSE als dat niet het geval is.

#### gTouches() \label{gtouches}

De vraag die we in deze paragraaf gaan beantwoorden is: *'Kun je een lijstje maken van alle gemeenten die aan de gemeente Utrecht grenzen?'* Natuurlijk is het antwoord hierop: *'Ja, dat kan ik.'*

Hieronder laten we zien hoe je dat met **R** kunt doen.

Maak eerst een object `utrecht` aan:

```{r}
utrecht <- gem2014[gem2014@data$Gemeentenaam == "Utrecht",]
```

Test nu voor elke gemeente of deze `utrecht` raakt, dat wil zegen: aan `utrecht` grenst. Let op: zet om elke gemeente afzonderlijk te testen `byid = TRUE`, anders wordt `gem2014` voor de vergelijking tot ??n geometrie - heel Nederland - samengevoegd.

```{r}
raakt_utrecht <- gTouches(utrecht, gem2014, byid = TRUE)
class(raakt_utrecht)
head(raakt_utrecht)
```

Het resultaat van deze test is een matrix met voor elk van de 403 gemeenten een waarde *FALSE* of een waarde *TRUE*.

Met `which()` kun je opvragen welke indices WAAR (TRUE) zijn:

```{r}
which(raakt_utrecht)
```

Aan alleen een lijstje met WAAR en ONWAAR hebben we natuurlijk niks, en daarom gaan we de functie `which()` gebruiken om een subset van aangrenzende gemeenten aan te maken:

```{r}
buurgemeenten_utrecht <- gem2014[which(raakt_utrecht),]
View(buurgemeenten_utrecht)
```

En ja, je kunt de subset ook in ??n stap aanmaken, en wel als volgt:

```{r}
buurgemeenten_utrecht2 <- gem2014[which(gTouches(utrecht, gem2014, byid = TRUE)),]
```

En laat maar zien op de kaart:

```{r}
plot(buurgemeenten_utrecht, col = "#FFCC66", border = "White",
     main = "gTouches(): Utrecht met buurgementen, 1 januari 2014")
plot(utrecht, col = "#FF6600", border = "White", add = TRUE)
text(coordinates(buurgemeenten_utrecht),
     as.character(buurgemeenten_utrecht@data$Gemeentenaam), cex = .3)
text(coordinates(utrecht), as.character(utrecht@data$Gemeentenaam),
     cex = .4, col = "white")
```

#### gOverlaps() \label{goverlaps}

De vraag in deze paragraaf is: *'Over welke gemeenten strekt de bevolkingskern Groot - Arnhem zich uit?'*

Voor het beantwoorden van deze vraag maken we gebruik van het bestand **bevolkingskern_2011** dat we eerder hebben gedownload (zie paragraaf \ref{shp}).

De tabel `gem2014` die we al in veel oefeningen hebben gebruikt, heeft betrekking op administratief afgebakende   gebieden. Deze administratieve eenheden houden echter geen of nauwelijks rekening met de fysiek ruimtelijke structuur van het Nederlands grondgebied. Sinds 2001 levert het CBS - als gevolg van eisen van Eurostat - weer statistieken voor fysiek afgebakende *urban areas*. Deze bevolkingskernen beslaan niet per definitie het gehele grondgebied van een gemeente, en ze kunnen zich over meerdere gemeenten uitstrekken. Voor meer achtergrondinformatie over de methodologie die door het CBS is gehanteerd bij het afbakenen van deze bevolkingsconcentraties, zie: [Vliegen et al. (2006)](http://www.cbs.nl/NR/rdonlyres/61D32380-5CFC-4CDC-8800-072EDEBAFCC1/0/2006bevolkingskerneninnederlandart.pdf).

Lees - indien nodig - het bestand **bevolkingskern_2011** in:

```{r eval=FALSE}
bevkern <- readOGR(dsn = "CBS", layer =  "bevolkingskern_2011")
```

De werkwijze hieronder lijkt sterk op de procedure die we voor `gTouches()` (zie paragraaf \ref{gtouches}) hebben gevolgd. 

Maak een object `groot_arnhem` aan:

```{r}
groot_arnhem <- bevkern[bevkern@data$KERN_NAAM == "Groot - Arnhem",]
```

Test nu voor elke gemeente of deze gedeeltelijk - of geheel - met `groot_arnhem` samenvalt:

```{r}
overlap_gr_ahm <- gOverlaps(groot_arnhem,gem2014, byid = TRUE)
```

Het systeem geeft aan dat de beide geometrie?n een verschillende `proj4string` hebben. Dit is geen foutmelding, maar slechts een *warning* (waarschuwing). Vraag: is dit een probleem dat opgelost moet worden of zijn er goede redenen om de waarschuwing alleen voor kennisgeving aan te nemen?

Gebruik vervolgens de functie `which()` om een subset van overlappende gemeenten aan te maken:

```{r}
gem_gr_ahm <- gem2014[which(overlap_gr_ahm),]
```

Desgewenst kun je de twee voorafgaande regels code samenvoegen in ??n stap:

```{r eval=FALSE}
gem_gr_ahm2 <- gem2014[which(gOverlaps(groot_arnhem,gem2014, byid = TRUE)),]
```

Toon het resultaat op de kaart:

```{r}
plot(gem_gr_ahm, col = "steelblue", border = "white",
     main = paste("gOverlaps(): De bevolkingskern Groot-Arnhem","\nbeslaat meerdere gemeenten"))
plot(groot_arnhem, col = rgb(1,0,0,.15), border = "red", add = TRUE)
text(coordinates(gem_gr_ahm), as.character(gem_gr_ahm@data$Gemeentenaam), cex = .3)
```

#### gIntersects()

We willen graag weten: *'Welke gemeenten liggen er aan de spoorlijn van Nijmegen naar venlo?'*

Als je de oefeningen met `gTouches()` (paragraaf \ref{gtouches}) en met `gOverlaps()` (paragraaf \ref{goverlaps}) hebt gedaan, dan zie je veel overeenkomsten met de stappen die we hieronder gaan zetten.

Maak een object `spoorlijn_nijmegen_venlo` aan:

```{r}
spoorlijn_nijmegen_venlo <- spoorwegen2015[spoorwegen2015@data$Traject == "Nijmegen - Venlo",]
```

Test nu voor elke gemeente of deze door de genoemde spoorlijn doorsneden wordt:

```{r}
intersect_nijm_ven <- gIntersects(spoorlijn_nijmegen_venlo, gem2014, byid = TRUE)
```

Gebruik de functie `which()` om een subset van doorsneden gemeenten aan te maken:

```{r}
gem_sp_nijm_ven <- gem2014[which(intersect_nijm_ven),]
```

En natuurlijk kun je de twee voorafgaande regels code samenvoegen in ??n stap:

```{r eval=FALSE}
gem_sp_nijm_ven2 <- gem2014[which(gIntersects(spoorlijn_nijmegen_venlo, gem2014, byid = TRUE)),]
```

Laat maar zien:

```{r}
plot(gem_sp_nijm_ven, col = "khaki2", border = "white",
     main = paste("gIntersects(): De spoorlijn van Nijmegen naar Venlo",
                  "\nloopt door meerdere gemeenten"))
plot(spoorlijn_nijmegen_venlo, col = "red", add = TRUE)
text(coordinates(gem_sp_nijm_ven),
     as.character(gem_sp_nijm_ven@data$Gemeentenaam), cex = .3)
```


\newpage

# Geografische objecten cre?ren

## Objecten samenvoegen op basis van attribuutgegevens: `gUnaryUnion()`

```{r}
prov2014 <- gUnaryUnion(gem2014, id = gem2014@data$Provincie_naam)
class(prov2014)
row.names(prov2014)
plot(prov2014, col = "darkgreen", border = "white")
text(coordinates(prov2014), row.names(prov2014), cex = .7, col = "red")
```
...
Er bestaan verschillende oplossingen voor het geocoderen van adressen en plaatsnamen met R. Vaak zijn deze oplossingen gebaseerd op commerci?le diensten (bijvoorbeeld Google) en maken ze gebruik van niet-vrije data.

In deze paragraaf kijken we naar een oplossing die gebruik maakt van - vrij beschikbare - OpenStreetMap gegegevens, namelijk `photon`. Wij zijn deze bibliotheek op het spoor gekomen via deze blog: [G?ocoder en masse avec R et sans Google Maps](http://rgeomatic.hypotheses.org/622).

Het *package* `phothon` biedt een **R** Interface naar de Photon API.
...
Om `photon` te kunnen installeren heb je `devtools` nodig (zie paragraaf \ref{dvtls}):
...
De auteur van dit boek heeft al wel deze blog geschreven: [raster(): het samenvoegen van rasterbestanden in R](http://www.twiav.nl/nl/blog0002.php)
...
+ Pebesma, Edzer & Bivand, Roger S. (2005). *Classes and Methods for Spatial Data: the sp Package*. E?n van de vignettes behorende bij de `sp` bibliotheek. URL: <https://cran.r-project.org/web/packages/sp/vignettes/intro_sp.pdf>
...
+ Vliegen, Mathieu, Van Leeuwen, Niek en Kerkvliet, Ferry (2006). *Bevolkingskernen in Nederland opnieuw afgebakend*. Centraal Bureau voor de Statistiek, Voorburg/Heerlen. Publicatiedatum CBS-website: 6 februari 2006. URL: <http://www.cbs.nl/NR/rdonlyres/61D32380-5CFC-4CDC-8800-072EDEBAFCC1/0/2006bevolkingskerneninnederlandart.pdf>
