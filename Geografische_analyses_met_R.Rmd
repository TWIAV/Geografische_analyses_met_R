...
header-includes:
   - \providecommand{\versionnumber}{0.4.3}
...
\item \textbf{Hyperlinks werken niet op papier:} dit document staat vol verwijzingen - zowel interne als externe - die in de electronische versie wel met één klik te volgen zijn.
...
#### Rijen èn Kolommen selecteren

Natuurlijk kun je ook een selctie maken van rijen èn kolommen. In het voorbeeld hieronder selecteren we een aantal kolommen met betrekking tot de grote gemeenten:
...
Natuurlijk kun je op deze wijze ook een selctie maken van rijen èn kolommen. In het voorbeeld hieronder selecteren we een aantal kolommen met betrekking tot de wel héééél erg kleine gemeenten:
...
Deze is ontwikkeld door Timothée Giraud en Nicolas Lambert van de \href{http://elementr.hypotheses.org/}{Groupe ElementR}. Let op de Franse uitspraak: \textbf{ElementR} - \textit{R pour les sciences humaines et sociales}.\par
...
Het data.frame `Gem_met_stations` bevat 398 observaties met 34 variabelen. Er zijn immers 398 stations, en voor elk station zijn alle 34 attribuutgegevens van de betreffende gemeente opgehaald. Sommige gemeenten komen meerdere keren voor, omdat er meer dan één station is.
...
Test nu voor elke gemeente of deze `utrecht` raakt, dat wil zegen: aan `utrecht` grenst. Let op: zet om elke gemeente afzonderlijk te testen `byid = TRUE`, anders wordt `gem2014` voor de vergelijking tot één geometrie - heel Nederland - samengevoegd.
...
En ja, je kunt de subset ook in één stap aanmaken, en wel als volgt:
...
Het systeem geeft aan dat de beide geometrieën een verschillende `proj4string` hebben. Dit is geen foutmelding, maar slechts een *warning* (waarschuwing). Vraag: is dit een probleem dat opgelost moet worden of zijn er goede redenen om de waarschuwing alleen voor kennisgeving aan te nemen?
...
Desgewenst kun je de twee voorafgaande regels code samenvoegen in één stap:
...
En natuurlijk kun je de twee voorafgaande regels code samenvoegen in één stap:
...
In deze paragraaf kijken we naar een oplossing die gebruik maakt van - vrij beschikbare - OpenStreetMap gegegevens, namelijk `photon`. Wij zijn deze bibliotheek op het spoor gekomen via deze blog: [Géocoder en masse avec R et sans Google Maps](http://rgeomatic.hypotheses.org/622).
