...
header-includes:
   - \providecommand{\versionnumber}{0.4.1}
...
Test nu voor elke gemeente of deze `utrecht` raakt, dat wil zegen: aan `utrecht` grenst. Let op: zet om elke gemeente afzonderlijk te testen `byid = TRUE`, anders wordt `gem2014` tot één geometrie - heel Nederland - samengevoegd. 
...
```{r}
bevkern <- readOGR(dsn = "CBS", layer =  "bevolkingskern_2011")
groot_amsterdam <- bevkern[bevkern@data$KERN_NAAM == "Groot - Amsterdam",]
overlap_gr_ams <- gOverlaps(groot_amsterdam,gem2014, byid = TRUE)
gem_gr_ams <- gem2014[which(overlap_gr_ams),]
```

Of - als je dat liever wilt - in één stap:

```{r eval=FALSE}
gem_gr_ams2 <- gem2014[which(gOverlaps(groot_amsterdam,gem2014, byid = TRUE)),]
```

```{r}
plot(gem_gr_ams, col = "steelblue", border = "white")
plot(groot_amsterdam, col = rgb(1,0,0,.15), border = "red", add = TRUE)
text(coordinates(gem_gr_ams), as.character(gem_gr_ams@data$Gemeentenaam), cex = .3)
```
